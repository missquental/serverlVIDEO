...
def main():
    ...
    with tab1:
        ...
        with col1:
            ...
            # Video selection with uploaded videos
            st.subheader("🎬 Select Video for Streaming")
            
            # Get local files
            video_files = [f for f in os.listdir('.') if f.endswith(('.mp4', '.flv', '.avi', '.mov', '.mkv'))]
            
            # Get uploaded videos from database
            uploaded_videos = get_uploaded_videos()
            video_options = {}
            
            # Add local files
            for file in video_files:
                video_options[file] = file
            
            # Add uploaded videos from database
            for video in uploaded_videos:
                video_id, filename, file_path, upload_time, file_size, duration = video
                if os.path.exists(file_path):
                    video_options[f"🗄️ {filename}"] = file_path
            
            # Video selection dropdown
            if video_options:
                selected_video_label = st.selectbox("Select video", list(video_options.keys()))
                video_path = video_options[selected_video_label]
                
                # If this is an uploaded video, store it in session state
                if selected_video_label.startswith("🗄️"):
                    st.session_state['selected_video_path'] = video_path
                    st.info(f"Using uploaded video: {selected_video_label}")
            elif 'selected_video_path' in st.session_state:
                video_path = st.session_state['selected_video_path']
                st.info(f"Using selected video: {os.path.basename(video_path)}")
            else:
                video_path = None
                st.info("No video files found. Please upload a video or place video files in the current directory.")
    ...
    with tab2:
        ...
        uploaded_file = st.file_uploader("Choose a video file", type=['mp4', '.flv', '.avi', '.mov', '.mkv'], key="video_library_uploader")
        ...
        # Tampilkan video yang sudah diupload
        st.subheader("📋 Your Video Collection")
        
        # Dapatkan video dari database
        ...
        if videos:
            ...
            if existing_videos:
                ...
                for idx, video in enumerate(existing_videos):
                    col_idx = idx % 3
                    col = cols[col_idx]
                    ...
                    with col:
                        ...
                        with st.container(border=True):
                            ...
                            # Preview video (thumbnail/preview singkat)
                            try:
                                # Tampilkan info file
                                st.caption(f"Size: {file_size:,} bytes")
                                if duration:
                                    st.caption(f"Duration: {timedelta(seconds=int(duration))}")
                                st.caption(f"Uploaded: {upload_time[:19].replace('T', ' ')}")
                                
                                # Tombol aksi
                                col1, col2, col3 = st.columns(3)
                                with col1:
                                    if st.button("🎬", key=f"use_{video_id}", help="Use for streaming"):
                                ...
                                with col3:
                                    if st.button("🗑️", key=f"delete_{video_id}", help="Delete video"):
                                        try:
                                            # Hapus file
                                            if os.path.exists(file_path):
                                                os.remove(file_path)
                ...
                st.info("No videos found. Upload some videos to get started!")
        else:
    ...
    # Log tabs
    tab_log1, tab_log2, tab_log3 = st.tabs(["🔴 Live Logs", "📊 Session History", "🗂️ All Logs"])
